...
class AboutJobItem extends Component {
  ...
  getJobsData = async props => {
    ...
    const responseJobData = await fetch(jobDataDetailsApiUrl, optionsJobDta)
    if (responseJobData.ok === true) {
      const fetchedJobData = await responseJobData.json()
      const updatedJobDetailsData = [fetchedJobData.job - details].map(
        eachItem => ({
  ...
  render() {
    return (
      <>
        <Header />
        <div className="jib-details-view-container">
